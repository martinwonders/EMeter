...
void collect_fast_streaming_triggered (void)
{
    ...
    if(!ps2000_set_channel(unitOpened.handle,PS2000_CHANNEL_A,TRUE,FALSE,PS2000_2V))
       fprintf(stderr,"Couldn't set channel A");

    if(!ps2000_set_channel(unitOpened.handle,PS2000_CHANNEL_B,FALSE,FALSE,PS2000_2V))
       printf(stderr,"Couldn't set channel B");
	...
	*/
	short over;
	ok = ps2000_run_streaming_ns ( unitOpened.handle, 200, 2, BUFFER_SIZE_STREAMING, 1, 1, 40000 );
	if(!ok) {
       fprintf(stderr,"There was an issue with the call to stream_ns...\n");
       return 1;
    }
	...
	*/
	...
	while (!kbhit () | !unitOpened.trigger.advanced.autoStop)
	{
	...
	//no_of_samples = ps2000_get_streaming_values_no_aggregation (unitOpened.handle, &startTime, // get samples from the beginning
	...
	//{
	//	printf ("%d ", adc_to_mv (values_a[i],0), unitOpened.channelSettings[0].range);
